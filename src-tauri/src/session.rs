...
pub struct Session {
    ...
    ocr_rect_debounce: Arc<Mutex<Option<std::thread::JoinHandle<()>>>>,
}
...
impl Session {
        ...
        let app2 = app.clone();
        {
            ...
            listener_ids.push(app.listen_any("original-watch-list-update", move |event| {
                if let Ok(payload) = serde_json::from_str::<Value>(event.payload()) {
                    ...
                    if !translate_watch_keys_evt.lock().contains_key(&key) {
    ...
    pub fn set_ocr_rect(&self, rect: Value) -> Result<(), String> {
        ...
        // 旧线程发现已有新任务就立即退出，由最新线程在安静 300ms 后写入。
        ...
        let handle = std::thread::spawn(move || {
            ...
            // 检查是否已被更新的请求替换（新线程持有 state 且是同一个）。
            let current = state.lock().as_ref().map(|h| h.thread().id());
            if current != Some(std::thread::current().id()) {
                // 已有更新的线程在等待，放弃本次写入。
                return;
            }
        ...
        *self.ocr_rect_debounce.lock() = Some(handle);
        Ok(())
    }
