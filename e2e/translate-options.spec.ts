...
test.describe('/options/translate-provider/腾讯混元大模型', () => {
  test('should display all options with correct fields and type tags', async ({ page }) => {
    await navigateTo(page, providerRoute('translate', '腾讯混元大模型'));
    await waitForContent(page);

    const mainContent = page.locator('#main-content');

    const title = mainContent.locator('.title');
    await expect(title).toContainText('腾讯混元大模型');
  ...
  test('should reject non-numeric input and accept valid number for maxHistory', async ({ page }) => {
    ...
    // chatConfig.maxHistory (Number): invalid "abc" → error, valid "10" → no error
    await testValidation(mainContent, 'chatConfig.maxHistory', 'abc', '应当是一个数字', '10');
  });
  ...
  test('should accept any string in [String, null] fields without error', async ({ page }) => {
    ...
    // [String, null] fields accept any string
  ...
  test('should open model select and show all enum options', async ({ page }) => {
    await navigateTo(page, providerRoute('translate', '腾讯混元大模型'));
    await waitForContent(page);

    const mainContent = page.locator('#main-content');
    // model is the second select (index 1)
    ...
    const options = page.locator('.t-select-option');
    const optionTexts = await options.allTextContents();
    expect(optionTexts).toContain('hunyuan-lite');
    expect(optionTexts).toContain('hunyuan-standard');
    expect(optionTexts).toContain('hunyuan-pro');
