...
export default defineOcrProvider({
    id: '腾讯云',
    description: '腾讯云通用印刷体识别API，详见 https://cloud.tencent.com/product/generalocr \n会向 ocr.tencentcloudapi.com 发送数据',
    optionsSchema: {
        enable: Boolean,
        apiConfig: {
            credential: {
            ...
            params: {
                LanguageType: {
                    type: String,
                    enum: [
                        'auto', 'zh', 'jap', 'kor', ' spa', 'fre', 'ger',
                        'por', ' vie', 'may', 'rus', 'ita', ' hol', 'swe',
                        'fin', 'dan', ' nor', 'hun', 'tha', 'lat', 'ara'
                    ]
                }
            }
        }
