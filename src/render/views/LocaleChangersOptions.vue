<template>
  <t-layout class="option-layout">
    <t-content class="option-content">
      <t-space class="option-content" direction="vertical">
        <t-skeleton v-if="updating" />
        <t-form v-else label-align="top">
          <t-form-item v-for="i in options" :key="i.name">
            <template #label>
              <t-input
                v-if="i.editingName"
                :value="i.name"
              ...
              <t-space v-else size="small">
                <span>{{ i.name }}</span>
                <t-button size="small" variant="text" @click="i.editingName = true">
                  <edit-icon />
                </t-button>
                <t-popconfirm content="确认删除？" @confirm="del(i)">
        ...
        <t-button variant="dashed" class="add" @click="add">
          <template #icon>
            <add-icon />
          </template>
          新建
    ...
    <t-footer v-if="options.length !== 0" class="option-footer">
      <t-space>
