  ...
  <div>
    <t-form>
      ...
      <t-form-item label="提取方法">
        <t-select :value="type" @change="(v) => changeType(v as Ame.Extractor.ExtractorType)">
          <t-option value="textractor" label="Textractor" />
          <t-option value="ocr" label="OCR" />
        </t-select>
      </t-form-item>
      <t-form-item v-if="type === 'textractor'" label="详细设置">
        <textractor-setting />
      </t-form-item>
      <t-form-item v-if="type === 'ocr'" label="">
        <t-button theme="primary" @click="openOcrGuideWindow">
          <template #icon>
            <refresh-icon />
          </template>
          再次启动Ocr引导界面
