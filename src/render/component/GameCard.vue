<template>
  <t-card class="game-card" hover-shadow>
    <template #title>
      <transition name="slide" mode="out-in">
        <div v-if="!showEdit" class="name">
          <t-image :src="icon" class="icon">
            <template #error>
              <file-icon />
            </template>
            <template #loading>
    ...
    <template #content>
      <transition name="slide" mode="out-in">
        <span v-if="!showEdit" class="path">
          {{ path }}
        </span>
        <t-form v-else :model="formState">
          ...
          <t-form-item label="HookCode">
            <t-input v-model:value="formState.hookCode" :disabled="formState.type !== 'textractor'" placeholder="" />
          </t-form-item>
          <t-form-item label="启动参数">
            <t-input v-model:value="formState.execShell" placeholder="" />
          </t-form-item>
        </t-form>
      </transition>
    </template>
    <template #actions>
      <t-space size="small">
        <t-button v-if="!showEdit" size="small" variant="text" @click="edit">
          <setting-icon />
        </t-button>
        <t-button v-else size="small" variant="text" @click="save">
          <save-icon />
        </t-button>
        <t-loading v-if="!showEdit" :loading="starting" inherit-color>
          <t-button size="small" variant="text" :disabled="starting" @click="play">
            <play-circle-stroke-icon />
          </t-button>
        </t-loading>
        <t-button v-else size="small" variant="text" @click="showEdit = false">
          <rollback-icon />
        </t-button>
        <t-popconfirm theme="danger" content="确认删除？" @confirm="$emit('del', id)">
  ...
  methods: {
    ...
    edit() {
      ...
      for (const i in formState) {
        if (!Object.hasOwn(formState, i)) continue;
