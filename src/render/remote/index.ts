 ...
 *
 * `intl-segmenter` is a browser API (`Intl.Segmenter`), so it is handled here
 * in the renderer (mirrors the old Electron worker-thread implementation).
 * `mecab` runs as a subprocess in the Rust backend. Both fall back to
 * per-character segmentation when no provider is enabled/ready.
 */
export async function segment(text: string): Promise<SegmentWord[]> {
  const manager = (await storeGet('segmentManager')) as { defaultProvider?: string } | undefined;
  const defaultProvider = manager?.defaultProvider ?? 'intl-segmenter';

  if (defaultProvider === 'intl-segmenter' && typeof Intl.Segmenter === 'function') {
    const opts = (await storeGet('segmentProviders.intl-segmenter')) as
      | { enable?: boolean; language?: string }
      | undefined;
    const enabled = opts?.enable ?? true;
    if (enabled) {
      const language = opts?.language ?? 'ja';
      const words = segmentByIntl(text, language);
      // If Intl.Segmenter produced nothing meaningful (e.g. only whitespace),
      // fall through to the Rust command's per-character fallback.
      if (words.some((w) => w.word.trim().length > 0)) {
        return words;
      }
    }
  }

  // mecab (and the per-character fallback) live in the Rust backend.
  return invoke<SegmentWord[]>('segment', { text });
}

/** Segment with `Intl.Segmenter` (word granularity), mirroring the old worker. */
function segmentByIntl(text: string, language: string): SegmentWord[] {
  const segmenter = new Intl.Segmenter(language, { granularity: 'word' });
  const words: SegmentWord[] = [];
  for (const { segment: word } of segmenter.segment(text)) {
    if (word.length > 0) {
      words.push({ word });
    }
  }
...
type TranslateWatchErrorCallback = (err: Ame.Translator.TranslateError, arg: Ame.Translator.TranslateResult) => void;
...
export async function watchTranslate(
  ...
  const unlisten2 = await listen<{ err: Ame.Translator.TranslateError; value: Ame.Translator.TranslateResult }>(
