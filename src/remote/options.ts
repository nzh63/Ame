...
export const getProviderOptionsMeta = defineRemoteFunction('get-provider-options-meta',
    async function <T extends ProvidersType>(event: Electron.IpcMainInvokeEvent, type: T, providerId: Awaited<ReturnType<typeof providers>>[T][number]['id']) {
...
export const getProviderOptions = defineRemoteFunction('get-provider-options', async (event, type: ProvidersType, providerId: string) => {
    const { default: store } = await import('@main/store');
    return store.get(`${type}Providers.${providerId}`);
});

export const setProviderOptions = defineRemoteFunction('set-provider-options', async (event, type: ProvidersType, providerId: string, value: unknown) => {
    const { default: store } = await import('@main/store');
...
export const getManagerOptionsMeta = defineRemoteFunction('get-manager-options-meta', async (event, type: ManagersType) => {
    return {
        id: null,
        description: null,
        ...(await managers())[type]
...
export const getManagerOptions = defineRemoteFunction('get-manager-options', async (event, type: ManagersType, _: unknown) => {
    const { default: store } = await import('@main/store');
    return store.get(`${type}Manager`);
});

export const setManagerOptions = defineRemoteFunction('set-manager-options', async (event, type: ManagersType, _: unknown, value: unknown) => {
    const { default: store } = await import('@main/store');
...
export const getExtractorOptionsMeta = defineRemoteFunction('get-extractor-options-meta', async (event, type: ExtractorsType, _: unknown) => {
    return {
        id: null,
        description: null,
...
export const getExtractorOptions = defineRemoteFunction('get-extractor-options', async (event, type:ExtractorsType, _: unknown) => {
    const { default: store } = await import('@main/store');
    return store.get(`${type}Extractor`);
});

export const setExtractorOptions = defineRemoteFunction('set-extractor-options', async (event, type: ExtractorsType, _: unknown, value: unknown) => {
    const { default: store } = await import('@main/store');
