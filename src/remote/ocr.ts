...
export const getScreenCapture = defineRemoteFunction('get-screen-capture', requireContext, async (event, force = false) => {
...
export const getPreprocessedImage = defineRemoteFunction('get-preprocessed-image', requireContext, async (event, img: Buffer, option: PreprocessOption) => {
    const window = electron.BrowserWindow.fromWebContents(event.sender) as WindowWithContext;
    if (!window.context.extractorTypeIs('ocr')) {
        throw new Error('Not in Ocr mode');
    } else {
...
export const getScreenCaptureCropRect = defineRemoteFunction('get-screen-capture-crop-rect', requireContext, (event) => {
    const window = electron.BrowserWindow.fromWebContents(event.sender) as WindowWithContext;
    if (!window.context.extractorTypeIs('ocr')) {
        throw new Error('Not in Ocr mode');
    } else {
...
export const setScreenCaptureCropRect = defineRemoteFunction('set-screen-capture-crop-rect', requireContext, (event, rect: sharp.Region) => {
    const window = electron.BrowserWindow.fromWebContents(event.sender) as WindowWithContext;
...
export const getScreenCapturePreprocessOption = defineRemoteFunction('get-screen-capture-preprocess-option', requireContext, (event) => {
    const window = electron.BrowserWindow.fromWebContents(event.sender) as WindowWithContext;
    if (!window.context.extractorTypeIs('ocr')) {
        throw new Error('Not in Ocr mode');
    } else {
...
export const setScreenCapturePreprocessOption = defineRemoteFunction('set-screen-capture-preprocess-option', requireContext, (event, option: PreprocessOption) => {
    const window = electron.BrowserWindow.fromWebContents(event.sender) as WindowWithContext;
