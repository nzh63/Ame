...
export const startExtract = defineRemoteFunction('start-extract', async (event, uuid: string, gamePids: number[], hookCode?: string, type?: Ame.Extractor.ExtractorType) => {
...
export const switchExtractorType = defineRemoteFunction('switch-extractor-type', requireContext, (event, type: Ame.Extractor.ExtractorType) => {
    const window = electron.BrowserWindow.fromWebContents(event.sender) as WindowWithContext;
