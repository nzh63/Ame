...
export default function replace(options: Options = { logFunction: {} }) {
  ...
  const pattern = new RegExp(
    `(?<![a-zA-Z0-9.])(${Object.keys(options.logFunction)
      .map((i) => escape(i))
      .join('|')})[\\b\\n]*\\([\\b\\n]*(['"\`]?)`,
    'g',
  );
  ...
  function codeRemove(code: string, id: string, magicString: MagicString) {
    ...
    function walk(node: ts.SourceFile, callback: (node: ts.Node) => boolean) {
      ...
      if (id.startsWith('@logger')) {
        if (options.disableLog !== true) {
          const namespace = /@logger\//.test(id)
            ? id.substring(id.lastIndexOf('@logger/') + '@logger/'.length).replace(/\//g, ':')
            : undefined;
